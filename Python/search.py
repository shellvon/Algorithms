...
def binarySearch(elements,key):
	...
	Description:
		...
		3) Else if x is greater than the mid element, then x can only lie in right half subarray after the mid element. So we find for right half.
	...
	find the key from the elements.the elements should be a sorted list,if not,would be return None
	"""
